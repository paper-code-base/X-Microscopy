...
#from evaluate_test import sample_file
from utils import *
...
class pix2pix(object):
    ...
    def train(self, args):
        ...
        counter = self.base_count
        data = glob('./{}/*'.format(self.dataset_name))
        batch_idxs = min(len(data), args.train_size) // self.batch_size
        start_epoch = math.ceil(counter / batch_idxs)
    ...
    def test_end(self, args):
        ...
        for sample_file in sample_files:
            print(sample_file)
            wf_dir = sample_file + '/wf'
            wf_filelist = get_filelist(wf_dir, [])
            MU_dir = sample_file + '/MU-SRM'
            MU_filelist = get_filelist(MU_dir, [])

            if len(wf_filelist) == 0  or  len(MU_filelist) == 0:
                continue
            else :
                wf_path = wf_filelist[0]
                MU_path = MU_filelist[0]
                graph = tf.Graph()
                with graph.as_default():
                    count = count+1
                    ...
                    if count <0:
                        continue
                    image1 = scipy.misc.imread(wf_path).astype('uint8')
                    ...
                    image2 = scipy.misc.imread(MU_path).astype('uint8')
                    ...
                    with tf.Session(graph=graph, config=tf.ConfigProto(
                        ...
                        image_b = np.array(sampleb[0]).astype(np.float32)
                        image_f = np.array(sample_fusion[0]).astype(np.float32)
                        folder = sample_file + '/XM3/'
                        # folder = ''
                        if not os.path.exists(folder):
                            os.makedirs(folder)

                        img_name_ak1 = folder + 'XM1'
                        img_name_ak2 = folder + 'XM2'
                        img_name_ak3 = folder + 'XM3'
                        save_images_val(image_a,
