...
parser.add_argument('--test_dataset_name', dest='test_dataset_name', default='../dataset/test-example-data', help='name of the test dataset')###########1
parser.add_argument('--epoch', dest='epoch', type=int, default=1000, help='the epoch')#############2
...
parser.add_argument('--withbn', dest='withbn', type=str2bool, default=True, help='model with or without bn, if withbn=True, the batch_size must be 1, else the batch_size can be set larger than 1, only True is supported now')#True
parser.add_argument('--same_input_size', dest='same_input_size', type=str2bool, default=True, help='mode input with same or different size')###########5
...
parser.add_argument('--resume', dest='resume', type=str2bool, default=False, help='(bool) resume training from last checkpoint')
...
def main(_):
    ...
    base_count = 0
    if args.resume: #resume training from last checkpoint
        ckpt = tf.train.get_checkpoint_state(args.checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            base_count = abs(int(ckpt.model_checkpoint_path.split("/")[-1]))
